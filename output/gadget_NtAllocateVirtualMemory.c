 ...
 * Call this ONCE before using SG_NtAllocateVirtualMemory().
 */

#include <windows.h>

QWORD SG_NtAllocateVirtualMemory_addr = 0;

BOOL SG_Init_NtAllocateVirtualMemory() {
    /* Step 1 - Get ntdll base address */
    HMODULE ntdll = GetModuleHandleA("ntdll.dll");
    if (!ntdll) return FALSE;

    /* Step 2 - Find the function in ntdll exports */
    FARPROC func = GetProcAddress(ntdll, "NtAllocateVirtualMemory");
    ...
    for (int i = 0; i < 32; i++) {
        if (ptr[i] == 0x0F && ptr[i+1] == 0x05 && ptr[i+2] == 0xC3) {
            ...
            SG_NtAllocateVirtualMemory_addr = (QWORD)(ptr + i);
            return TRUE;
        }
    }
