 ...
 * Call this ONCE before using SG_NtProtectVirtualMemory().
 */

#include <windows.h>

QWORD SG_NtProtectVirtualMemory_addr = 0;

BOOL SG_Init_NtProtectVirtualMemory() {
    /* Step 1 - Get ntdll base address */
    HMODULE ntdll = GetModuleHandleA("ntdll.dll");
    if (!ntdll) return FALSE;

    /* Step 2 - Find the function in ntdll exports */
    FARPROC func = GetProcAddress(ntdll, "NtProtectVirtualMemory");
    ...
    for (int i = 0; i < 32; i++) {
        if (ptr[i] == 0x0F && ptr[i+1] == 0x05 && ptr[i+2] == 0xC3) {
            ...
            SG_NtProtectVirtualMemory_addr = (QWORD)(ptr + i);
            return TRUE;
        }
    }
